...
function install_chaincode(){
    docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@manu.meditrack.com/msp" peer0.manu.meditrack.com peer chaincode install -l node -n exam6 -p /etc/hyperledger/chaincode/ -v v0
...
function instantiateChaincodeWithPrivate(){
    ...
    docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@manu.meditrack.com/msp" peer0.manu.meditrack.com peer chaincode instantiate -l node -o orderer.meditrack.com:7050 -C ourchannel -n exam6 -v v0 -c '{"Args":["init"]}' --collections-config privateDataCollectionProfile.json
}

function invokePrivateData(){
    docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@manu.meditrack.com/msp" peer0.manu.meditrack.com peer chaincode invoke -o orderer.meditrack.com:7050 -C ourchannel -n exam6  -c '{"Args":["setPrivateMedicinePrice"]}'
}

function queryPrivateData(){
    docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@manu.meditrack.com/msp" peer0.manu.meditrack.com peer chaincode query -C ourchannel -n exam6 -c '{"Args":["getPrivateMedcinePrice"]}'
}
