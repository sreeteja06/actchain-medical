...
describe('MedicineContract', () => {
    ...
    beforeEach(() => {
        ...
        ctx = new TestContext();
        ctx.stub.getState.withArgs('001').resolves(Buffer.from('{"name":"critic","owner":"manu","expDate":"10/08/2019","location":"hyd","extraConditions":{"temp":{"required":"45c","present":""}}}'));
    ...
    describe('#update extra condition', () => {

        it('update the extra condition', async () => {
            await contract.updateExtraCondition(ctx, '001', 'temp', '20c');
            const x = await contract.readMedicine(ctx, '001');
            console.log(x);
        });
    });
